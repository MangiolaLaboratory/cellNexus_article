...
```{r}
# Downaload from Zenodo (https://zenodo.org/records/19909778)
output_parquet = "cell_annotation.parquet"

# load non immune harmonisation
non_immune = read.csv("cell_type_harmonisation_non_immune.csv")
...
map_files = list.files("graphs", pattern = "immune_map", full.names = TRUE)
...
```{r}
immune_adj = read.csv("graphs/immune_tree.csv", check.names = FALSE, row.names = 1)
...
write.csv(level_map, "caq_celltype_level_map.csv", row.names = FALSE)
```
