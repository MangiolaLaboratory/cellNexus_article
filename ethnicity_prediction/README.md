...
- `etnicity_pipeline_targets.R`  
  Optional upstream preprocessing pipeline that regenerates the processed pseudobulk object from source. This is the full reproducibility route, but it requires a SLURM-based HPC cluster (job scheduling via `crew.cluster`).
...
If you want to regenerate the processed pseudobulk object from source, run `etnicity_pipeline_targets.R` first and then follow the same steps above. This route is optional and requires a SLURM-based HPC cluster.
...
`etnicity_pipeline_targets.R` is the full regeneration route but requires a SLURM-based HPC cluster with `crew.cluster`, `cmdstanr`, and access to the original storage paths.
