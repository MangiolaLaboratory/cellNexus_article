...
cols <- colnames(se)
  ...
  try({
    saveRDS(se_pb, "pseudobulk_se_X_Y_subset_sample_id.rds")
  }, silent = TRUE)
...
# Download pseudobulk_se.h5ad from https://zenodo.org/records/17060800

se_raw <- zellkonverter::readH5AD("pseudobulk_se.h5ad", reader = "R", use_hdf5 = TRUE)
...
# Save the splits
saveRDS(splits, "sex_prediction_splits.rds")
```

```{r read splits}
splits <- readRDS("sex_prediction_splits.rds")
...
if (!exists("sex_model_results")) {
  sex_model_results <- readRDS("sex_prediction_model.rds")
}

# Predict sex for all processed pseudobulk samples
se_all_pred <- predict_sex_for_se(
  se = splits$se_processed,
...
all_sex_predictions <- colData(se_all_pred) |>
  as.data.frame() |>
  as_tibble() |>
  dplyr::select(
    sample_id,
...
write.csv(all_sex_predictions, "sex_prediction_all_samples.csv", row.names = FALSE)
...
plot_unknown_prediction <- 
  ...
  colData(se_unknown_filtered) |> 
    ...
    dplyr::count(tissue_groups, sex_predicted) |>
    arrange(desc(n)) |>
    write.csv("unknown_prediction_counts.csv", row.names = FALSE)
