...
sample_to_cell_primary_human <- sample_to_cell_primary |>
  ...
  select(
    observation_joinid, cell_, sample_,
...
# accepted_assays from census of July 2024 version
accepted_assays <- read.csv("metadata/census_accepted_assays_2024-07-01.csv")
colnames(accepted_assays) <- c("id", "assay")

sample_to_cell_primary_human_accepted_assay <- sample_to_cell_primary_human |>
...
large_samples_collection_id <- large_samples |>
  ungroup() |>
...
collection_ids <- large_samples_collection_id |>
  collect() |>
  pull(collection_id)
...
selected_columns <- c(
  "assay", "disease", "donor_id", "sex", "self_reported_ethnicity", "tissue", "development_stage", "is_primary_data", "dataset_id", "observation_joinid",
...
census_samples_to_download |>
  dplyr::count(observation_joinid, sample_2) |>
  dplyr::count(n)
census_samples_to_download |>
  dplyr::count(cell_, sample_2) |>
  dplyr::count(n)

# light version
census_samples_to_download |>
  group_by(dataset_id, sample_2) |>
