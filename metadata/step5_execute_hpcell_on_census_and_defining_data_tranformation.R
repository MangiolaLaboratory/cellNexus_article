...
downloaded_samples_tbl <- downloaded_samples_tbl |>
  ...
  mutate(
    cell_number = cell_number |> as.integer(),
...
sample_tbl <- sample_tbl |>
  left_join(sample_meta, by = "dataset_id") |>
  distinct(
    file_name, tier, cell_number, dataset_id, sample_2,
    x_normalization, x_approximate_distribution
  ) |>
  mutate(transform_method = case_when(
    str_like(x_normalization, "C%") ~ "log",
  ...
  mutate(method_to_apply = case_when(
    transform_method %in% c("log", "LogNormalization", "LogNormalize", "log-normalization") ~ "exp",
    ...
    transform_method %in% c(
      "log1p", "log1p, base e", "Scanpy",
      "scanpy.api.pp.normalize_per_cell method, scaling factor 10000"
    ) ~ "expm1",
  ...
  mutate(comment = case_when(
    str_like(x_normalization, "Counts%") ~ "a checkpoint for max value of Assay must <= 50",
...
sample_tbl <- sample_tbl |> mutate(
  count_upper_bound = 20,
...
sample_summary_df <- sample_summary_df |> impute_x_approximate_distribution() |>
  ...
  mutate(method_to_apply = case_when(
    inferred_distribution == "log" ~ "exp",
    inferred_distribution == "log1p" ~ "expm1",
    inferred_distribution == "raw" ~ "identity"
  ))

sliced_sample_tbl <- sliced_sample_tbl |>
  left_join(sample_summary_df |> mutate(sample_id = str_remove(sample_id, ".h5ad")) |>
    select(sample_id, method_to_apply), by = c("sample_2" = "sample_id")) |>
  mutate(method_to_apply = case_when(
    is.na(method_to_apply.y) ~ method_to_apply.x,
...
cell_type_concensus_tbl <- cell_type_concensus_tbl |> mutate(
