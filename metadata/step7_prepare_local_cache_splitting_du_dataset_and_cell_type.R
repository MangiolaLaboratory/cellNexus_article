...
copy_query <- "
  ...
  ) TO  'cell_metadata_cell_type_consensus_v1_8_0_mengyuan.parquet' -- MODIFY HERE: output merged metadata parquet
...
job::job({
  ...
  CREATE VIEW cell_metadata AS
  SELECT *
  FROM read_parquet('cell_metadata_cell_type_consensus_v1_8_0_mengyuan.parquet')
")
  ...
  SELECT *
  FROM read_parquet('file_id_cell_id_dict_v1_3_0_Jul_2024.parquet')
")
  ...
  ) TO  'cell_metadata_cell_type_consensus_v1_8_1_mengyuan.parquet' -- MODIFY HERE: output final metadata parquet with new cell IDs (v1_3_2)
  (FORMAT PARQUET, COMPRESSION 'gzip');
"
...
cell_metadata = 
  tbl(
    dbConnect(duckdb::duckdb(), dbdir = ":memory:"),
    sql("SELECT * FROM read_parquet('cell_metadata_cell_type_consensus_v1_8_1_mengyuan.parquet')")
  )

library(targets)
library(tidyverse)
store_file_cellNexus = "targets_prepare_database_split_datasets_chunked_1_8_0_single_cell" # MODIFY HERE: targets store directory for this pipeline
...
tar_script({
  ...
  list(
    ...
    tar_target(cache_directory, "cellNexus/hca_2024/0.5.0", deployment = "main"), # MODIFY HERE: output cache directory for saved anndata files
    tar_target(
      cell_metadata,
      "cell_metadata_cell_type_consensus_v1_8_1_mengyuan.parquet", # MODIFY HERE: final metadata parquet (should match the COPY TO output above)
    ...
    tar_target(
      cell_id_dict,
      "file_id_cell_id_dict_v1_3_0_Jul_2024.parquet", # MODIFY HERE: cell_id dictionary parquet
      packages = c( "arrow","dplyr","duckdb")
    ),
    ...
    tar_target(
      ...
      {
        cells_to_remove_parquet <- "cells_to_remove_in_metadata_Jul_2024.parquet"
        output_parquet <- "cell_metadata_cell_type_consensus_v1_8_1_filtered_missing_cells_mengyuan.parquet" # MODIFY HERE: output parquet after filtering missing cells
