# This script builds the cellNexus local single-cell cache from the HPCell QC
# output produced in step 6, and runs in four sequential stages. (1) File-ID
# generation: cells are grouped into sample and cell chunks and assigned stable
# hash-based IDs — single-cell file IDs encode dataset_id, sample chunk, cell
# chunk, and original Census cell_type; pseudobulk file IDs encode dataset_id
# and sample chunk — written to parquet. (2) Metadata merge: cell-level
# metadata, HPCell QC annotations, consensus cell-type labels, file IDs, and
# sample count-transformation methods are unified into a single metadata parquet
# via DuckDB SQL joins; a second pass swaps in compact cell IDs from the
# dictionary built in step 7 supplementary. (3) Cache writing: a
# {targets}/{crew.cluster} SLURM pipeline reads HPCell SCE and SCTransform
# targets, groups cells by file-ID chunk, and writes four expression layers —
...
job::job({
  ...
  get_file_ids(
    "cell_annotation_2024_Jul.parquet" # MODIFY HERE: input cell annotation parquet
  )  |> 
    write_parquet("file_id_cellNexus_single_cell_2024_Jul.parquet") # MODIFY HERE: output file_id parquet
  
  gc()
  
  con <- dbConnect(duckdb::duckdb(), dbdir = ":memory:")
  
  dir.create(file.path(tempdir(), "duckdb_tmp"), showWarnings = FALSE)
  
  DBI::dbExecute(
    con,
    glue::glue("SET temp_directory='{file.path(tempdir(), \"duckdb_tmp\")}';")
  )
  ...
  FROM read_parquet('metadata_cellxgenedp_Apr_2024/cell_metadata.parquet')
")
  ...
  FROM read_parquet('cell_annotation_2024_Jul.parquet')
")
...
dbExecute(con, "
  ...
  FROM read_parquet('file_id_cellNexus_single_cell_2024_Jul.parquet')
")
...
dbExecute(con, "
  ...
  FROM read_parquet('updated_transform_sample_tbl_2024_Jul.parquet')
")
...
copy_query <- "
  ...
  ) TO  'cell_metadata_cell_type_consensus_v1_7_0_mengyuan.parquet' -- MODIFY HERE: output merged metadata parquet
...
# cell_map is processed in a separate target script step7_supp_dataset_cell_map.R
job::job({
  con <- dbConnect(duckdb::duckdb(), dbdir = ":memory:")
  ...
  CREATE VIEW cell_metadata AS
  SELECT *
  FROM read_parquet('cell_metadata_cell_type_consensus_v1_7_0_mengyuan.parquet')
")
  ...
  SELECT *
  FROM read_parquet('file_id_cell_id_dict_v1_2_0_Jul_2024.parquet')
")
  ...
  ) TO  'cell_metadata_cell_type_consensus_v1_7_1_mengyuan.parquet' -- MODIFY HERE: output final metadata parquet with new cell IDs (v1_3_2)
  (FORMAT PARQUET, COMPRESSION 'gzip');
"
...
cell_metadata = 
  tbl(
    dbConnect(duckdb::duckdb(), dbdir = ":memory:"),
    sql("SELECT * FROM read_parquet('cell_metadata_cell_type_consensus_v1_7_1_mengyuan.parquet')")
  )

library(targets)
library(tidyverse)
store_file_cellNexus = "targets_prepare_database_split_datasets_chunked_1_7_0_single_cell" # MODIFY HERE: targets store directory for this pipeline
...
tar_script({
  ...
  list(
    ...
    tar_target(my_store, "2024-07-01/process_updated_samples_transform_hpcell_target_store_v1", deployment = "main"), # MODIFY HERE: HPCell targets store to read SCEs from
    tar_target(cache_directory, "cellNexus/hca_2024/0.4.1", deployment = "main"), # MODIFY HERE: output cache directory for saved anndata files
    tar_target(
      cell_metadata,
      "cell_metadata_cell_type_consensus_v1_7_1_mengyuan.parquet", # MODIFY HERE: final metadata parquet (should match the COPY TO output above)
    ...
    tar_target(
      cell_id_dict,
      "file_id_cell_id_dict_v1_2_0_Jul_2024.parquet", # MODIFY HERE: cell_id dictionary parquet
      packages = c( "arrow","dplyr","duckdb")
    ),
...
#missing_cells_tbl |> write_parquet("cells_to_remove_in_metadata_Jul_2024.parquet")
con <- dbplyr::remote_con(cell_metadata)  

missing_cells_tbl <- dplyr::tbl(
  con,
  dplyr::sql("SELECT * FROM read_parquet('cells_to_remove_in_metadata_Jul_2024.parquet')")
)
...
DBI::dbExecute(
  con,
  glue::glue(
    "COPY ({query_sql}) TO 'cell_metadata_cell_type_consensus_v1_7_1_filtered_missing_cells_mengyuan.parquet' (FORMAT PARQUET, COMPRESSION ZSTD)"
  )
