...
tar_script(
  {
    ...
    get_dataset_id <- function(target_name, my_store) {
      ...
      if (sce |> is.null()) {
        return(tibble(
          sample_id = character(), dataset_id = character(),
      ...
      dataset_id_sample_id |>
        left_join(
          tbl(
            ...
            distinct(
              sample_id, sample_pseudobulk_chunk, cell_chunk,
    ...
    cbind_sce_by_dataset_id <- function(target_name_grouped_by_dataset_id,
      ...
      file_id_db <-
        ...
        select(
          sample_id, dataset_id, cell_type_unified_ensemble,
          file_id_cellNexus_pseudobulk
        )
    ...
    save_anndata <- function(dataset_id_sce, cache_directory) {
      ...
      .x <- dataset_id_sce |>
        ...
        _[[1]]
      .y <- dataset_id_sce |>
        pull(file_id_cellNexus_pseudobulk) |>
        _[[1]] |>
        str_remove("\\.h5ad")
