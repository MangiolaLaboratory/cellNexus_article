...
selected_columns <- c(
  "assay", "assay_ontology_term_id", "disease", "disease_ontology_term_id",
...
dataset_ids_path <- samples |>
  distinct(dataset_id) |>
  # head(2 ) |>
  mutate(download_path = paste0("aws s3 cp --no-sign-request ", h5ads.uri, dataset_id, ".h5ad ", path)) |>
