...
library(tidyr)
source("/Users/shen.m/projects/cellNexus/preprint/plot_custom_theme.R")
source("/Users/shen.m/projects/cellNexus/preprint/CAQ_age_analysis_functions.R")
```

# Load and process data
```{r}
cell_metadata <- get_metadata(get_metadata_url(databases = "metadata.1.0.12.parquet"))
...
pseudobulk = HDF5Array::loadHDF5SummarizedExperiment("~/projects/cellNexus/preprint/data/doublet_de_1_0_12/")
...
```{r}
se_filtered = se |> keep_abundant(minimum_count_per_million = 30) |> scale_abundance()
...
tbl = se_filtered |> 
  ...
  mutate(symbol = AnnotationDbi::mapIds(org.Hs.eg.db::org.Hs.eg.db, 
                                        ...
                                        column="SYMBOL", 
                                        multiVals = "first")) |> 
  distinct(.feature, symbol, logFC, logCPM, F, PValue, FDR, .feature) |> 
  as_tibble() 
```
