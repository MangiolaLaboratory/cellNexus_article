...
top_tissues_in_donors <- cell_metadata |>
  ...
  collect() |>
  arrange(desc(donor_count)) |> head() |> pull(tissue_groups)

# Disease groups has little donors
diseases_to_exclude <- cell_metadata |> group_by(disease_groups_coarse) |> 
  summarise(n = n_distinct(donor_id)) |> arrange(n) |> 
  collect() |> head(3) |> pull( disease_groups_coarse)
...
tissue_disease_pairs = cell_metadata |>
  ...
  mutate(disease_groups_coarse = as.factor(disease_groups_coarse),
...
tech_order = tissue_disease_pairs |>  group_by(assay_groups) |>
  summarise(total = sum(donor_count)) |>
  arrange(desc(total)) |>
  pull(assay_groups)
...
tissue_disease_pairs = tissue_disease_pairs |> mutate(tissue_text_size = case_when(tissue_groups == "Blood" ~ 4,
                               ...
                               assay_text_size = case_when(assay_groups == "10x Genomics 3" ~ 2.5,
                               ...
                               disease_text_size = case_when(disease_groups_coarse == "Normal" ~ 3,
...
tissue_size <- tissue_disease_pairs |> distinct(tissue_groups, tissue_text_size) |> deframe() 
disease_size <- tissue_disease_pairs |> distinct(disease_groups_coarse, disease_text_size) |> deframe()
assay_size <- tissue_disease_pairs |> distinct(assay_groups, assay_text_size) |> deframe()
...
tissue_disease_alluvial <- tissue_disease_pairs |> 
  ggplot(aes(axis1 = tissue_groups, axis2 = disease_groups_coarse, 
             y = donor_count)) +
...
tissue_assay_alluvial <- tissue_disease_pairs |> 
  ggplot(aes(axis1 = assay_groups, axis2 = tissue_groups, 
             y = donor_count)) +
  geom_alluvium(aes(fill = tissue_groups), width = 0.5, show.legend = F) +
