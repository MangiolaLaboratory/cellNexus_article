...
donor_metadata = cell_metadata |> 
  distinct(donor_id, self_reported_ethnicity, age_days, sex, tissue_groups) |> as_tibble()

age_groups_vector = coarse_age_bin(age_days = donor_metadata |> pull(age_days), sex = donor_metadata |> pull(sex))
donor_metadata = donor_metadata |> mutate(age_groups = age_groups_vector)  |> 
...
ethnicity_colors <- setNames(npg_cols, 
                             c("European",
                               "East Asian",
                               "Japanese",
                               "South Asian",
