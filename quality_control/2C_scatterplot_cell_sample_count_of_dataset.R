...
tech_cols <- c("10x Genomics 3"="#a6cee3",
...
dataset_summary <- dataset_summary %>% as_tibble() %>% 
...
label_data <- dataset_summary |> 
  ...
  mutate(author = case_when(explorer_url == "https://cellxgene.cziscience.com/e/d567b692-c374-4628-a508-8008f6778f22.cxg/" ~ "Kanemaru et al",
...
dataset_tech_df = cell_metadata |> distinct(dataset_id, assay_groups) |> collect()
...
alive_per_donor_scatter <- dataset_summary |> 
  ...
  scale_color_manual(values = tech_cols,
